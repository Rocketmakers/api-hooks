...
export namespace ApiHooksCaching {
   ...
   * @param staleIfOlderThan Cached data will be re-fetched when older than this value
   * @param staleOnErrorIfOlderThan Cached data will be available in error state if older than this value
   * @param fetchInBackground If true - the stale data will still be available whilst the new data is being fetched
   * @param bookmarkParameters An optional array of params to mark as "bookmarks", meaning that if a request is made with a falsy value in one of these params, the value from the previous request will be used if it exists. This is useful primarily for paging cursors.
   */
   ...
   * @param staleIfOlderThan 30000 - Cached data for a specific endpoint/param combination will be used for 30 seconds, and then re-requested the next time it's needed.
   * @param staleOnErrorIfOlderThan 'never' - Cached data will always be used if the request is in an error state
   * @param fetchInBackground true - Cached data will be available whilst the latest is being fetched - a spinner should be shown instead of the stale data where appropriate.
   */
